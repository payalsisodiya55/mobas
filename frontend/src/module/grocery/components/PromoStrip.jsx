...
export default function PromoStrip({ activeTab = "all" }) {
  ...
  const [subcategoryImagesMap, setSubcategoryImagesMap] = useState({});
  ...
  const fetchSubcategoryImages = useCallback(async (cards) => {
        ...
        if (response.success && response.data) {
          ...
          if (response.data.promoStrip && response.data.promoStrip.isActive) {
            ...
            if (promoStrip.featuredProducts && promoStrip.featuredProducts.length > 0) {
              fetchedProducts = promoStrip.featuredProducts.map((p) => {
          ...
          if (fetchedProducts.length === 0 && response.data.bestsellers && response.data.bestsellers.length > 0) {
            fetchedProducts = response.data.bestsellers.map((p) => {
