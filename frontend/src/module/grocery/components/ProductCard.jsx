...
// StarRating and Badge components (if not imported, we use local mocks for now or import them)
const StarRating = ({ rating, reviewCount }) => <div className="text-[10px] text-yellow-500">★ {rating} ({reviewCount})</div>;
const Badge = ({ children, variant, className }) => <span className={`px-1.5 py-0.5 rounded text-[10px] ${className}`}>{children}</span>;
const Button = ({ children, onClick, className, disabled, variant, size, ref }) => (
  <button ref={ref} onClick={onClick} disabled={disabled} className={`rounded-lg ${className}`}>
    {children}
  </button>
);
...
export default function ProductCard({
    ...
    const checkWishlist = async () => {
      try {
        ...
        if (res.success && res.data && res.data.products) {
          ...
          const exists = res.data.products.some(p => String(p._id || p.id) === targetId);
  ...
  const cartItem = cart.items.find((item) => item?.product && (item.product.id === product.id || item.product._id === product.id || item.product.id === product._id));
  const inCartQty = cartItem?.quantity || 0;

  // Get Price and MRP using utility
  const { displayPrice, mrp, discount } = { displayPrice: product.price || 0, mrp: product.mrp || 0, discount: 0 };
  ...
  const handleDecrease = async (e) => {
    ...
    try {
      await updateQuantity((product.id || product._id), inCartQty - 1);
    } finally {
      // Reset the flag after the operation truly completes
      isOperationPendingRef.current = false;
    }
            ...
            <>
              ...
              <div className="mb-0.5">
                <StarRating
                  ...
                  reviewCount={(product.reviews || product.reviewsCount) || 0}
                  size="sm"
                  showCount={true}
                />
              </div>
